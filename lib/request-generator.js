...
class RequestGenerator {
  ...
  constructor(url, tick, options) {
    ...
    if (tick <= this.timeout) {
      ...
      throw new Error("RequestGenerator: tick value must be strictly greater than curl timeout value(" + this.timeout + "ms)");
    }
    ...
    this.intervalPerform = setInterval(
      () => {
        try {
          ...
          // fail silently?
          console.log("RequestGenerator: error in curl request, handle already running");
        }
      },
      this.tick
    )
