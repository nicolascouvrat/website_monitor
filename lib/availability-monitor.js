...
class AvailabilityMonitor {
  constructor (url, options) {
    ...
    this.fileName = (options.fileName || this.trimUrl(url)) + ".log.txt";
    this.filePath = options.path || './.availability_log/';
   ...
   * Turns off file saving if error occurs.
   * @param {String}
   */
  saveInFile(string) {
    fs.appendFile(this.filePath + this.fileName, string + "\n", err => {
      if (err) {
        console.error("AvailabilityMonitor: error when saving file ", this.fileName, ": ", err);
        console.info("Unable to set up log file for website {", this.url, "}. Subsequent messages will not be stored.");
