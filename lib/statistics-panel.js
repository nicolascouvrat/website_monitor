...
class StatisticsPanel {
  ...
  addScan(delay, amplitude) {
    if (!Number.isInteger(delay)) {
      throw new TypeError("StatisticsPanel: scan delay must be an integer!");
    }
    if (!Number.isInteger(amplitude)) {
      throw new TypeError("StatisticsPanel: scan amplitude must be an integer!");
    }
  ...
  /**
   * Destroys all intervals and stops scans
   * @public
   */
  stop() {
  ...
  /**
   * Resume scanning after it has been stopped (just an alias to .start())
   * @public
   */
  resume() {
    this.start();
